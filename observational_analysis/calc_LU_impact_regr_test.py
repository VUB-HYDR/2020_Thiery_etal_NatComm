...
def calc_LU_impact_regr_test(model,exp,seas,yr_start1,yr_end1,yr_start2,yr_end2):
  #Run other programs needed in the calculation
  Tmean_out = T_mean_seas(model,yr_start1,yr_end1,yr_start2,yr_end2)
  ...
  return LU_impact_mean

modelname = 'MPI-ESM-MR'

if modelname == 'GFDL-CM3':
  dTmean_def = np.zeros((5,4,90,144))
elif modelname == 'HadGEM2-ES':
  dTmean_def = np.zeros((4,4,145,192))
elif modelname == 'IPSL-CM5A-LR':
  dTmean_def = np.zeros((6,4,96,96))
elif modelname == 'IPSL-CM5A-MR':
  dTmean_def = np.zeros((3,4,143,144))
elif(modelname == 'MPI-ESM-LR' or modelname == 'MPI-ESM-MR'):
  dTmean_def = np.zeros((3,4,96,192))


for k in range(0,dTmean_def.shape[0]):
  for l in range(0,4):
    dTmean_def[k,l,:,:] = calc_LU_impact_regr_test(modelname,sim_list[k],seas_list[l],1861,1890,1975,2004)